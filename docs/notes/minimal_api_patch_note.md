...
- 新增最小 HTTP transport adapter：`backend/api/minimal_api.py`
...
- Hybrid retrieval 仍由 `backend/retrieval/hybrid.py` 负责
...
```bash
./.venv/bin/python -m backend.api.minimal_api --smoke
```
