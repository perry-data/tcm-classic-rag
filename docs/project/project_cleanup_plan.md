...
- 当前数据准备主链：`scripts/check_dataset_acceptance.py` -> `scripts/build_v1_safe_dataset.py` -> `build_v1_database.py` -> `build_dense_index.py`。
...
| `build_v1_database.py` | A | - | 当前 SQLite 落库入口。 |
...
| `artifacts/zjshl_v1.db` | C | rebuildable_artifact | 当前仍在使用的运行产物；可重建，但本轮不建议移除。 |
...
| `dist/zjshl_dataset_v2_v1_safe.zip` | A | rebuildable_artifact | 当前 safe 数据包基线。 |
...
| `scripts/build_v1_safe_dataset.py` | A | - | 当前 safe 数据包构建入口。 |
...
- `dist/zjshl_dataset_v2_v1_safe.zip` 可由 `scripts/build_v1_safe_dataset.py` 重建。
- `artifacts/zjshl_v1.db`、`artifacts/database_*` 可由 `build_v1_database.py` 重建。
...
- `docs/05_dataset_patch_note.md`、`reports/dataset_patch_summary.json` 可由 `scripts/build_v1_safe_dataset.py` 重建。
...
- `run_minimal_retrieval.py`、`layered_enablement_policy.json`、`database_schema_draft.json`、`data/processed/zjshl_dataset_v2/`、`dist/zjshl_dataset_v2_v1_safe.zip`、`artifacts/zjshl_v1.db` 不能被误判为旧文件。
