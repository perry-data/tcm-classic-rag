...
def _detect_query_style(query_text: str) -> str:
    ...
    if any(marker in compact for marker in ("是什么意思", "什么意思")):
        ...
        if query_style == "meaning_explanation":
            lines.extend(
                [
                    ...
                    "【高频问法特化：问古文或术语“是什么意思”】",
                    "请按以下顺序自然作答：",
                    "1. 先给白话：用一句话说明它在提醒什么（例如：做法导致的结果/利弊）。",
                    "2. 再解释关键词：说明具体词汇（如“益阳/损阴”或“卫阳/荣阴”）的含义。",
                    "3. 阐述不确定点：说明是因为缺上下文还是只命中辅助材料导致的不确定。",
                    "4. 给核对路径：建议用户回看上一句、下一句或同段条文，并附上引用。",
                ]
            )
        return "\n".join(lines)
...
def _system_instruction(*, strict_retry: bool) -> str:
    ...
    if strict_retry:
        instruction += (
            ...
            "不要使用没有证据支撑的抽象总结；"
            "不要写成 1. 2. 3. 的机械罗列；"
            "绝对禁止输出“结论：”、“解释：”、“依据：”等标题，禁止输出“当前只输出弱表述”之类内部话术；"
    ...
    blocks.extend(
        [
            ...
            (
                ...
                "8. 绝对禁止输出“当前只输出弱表述”、“主证据优先”这类系统内部设定或日志话术。\n"
                ...
                "14. 只输出 JSON，不要输出 markdown 代码块。JSON 至少包含："
