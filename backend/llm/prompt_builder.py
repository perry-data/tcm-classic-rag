...
def _build_mode_style_block(answer_mode: str, query_style: str) -> str:
    if answer_mode == "strong":
        lines = [
            ...
            "1. 首段（必须先回答）：先用一句自然中文直接回答用户问句，先把“大意是什么/该怎么理解”讲明白。",
            "2. 第二段（解释关系）：只解释最关键的 1~2 个词、对举关系、因果关系或提醒重点；把依据揉进解释句，不要另起“依据段”。",
            "3. 如确有必要才补第 3 段：补一句语境说明或范围提醒，不要把正文写成证据摘要。",
        ]
        if query_style == "formula_identity":
            lines.extend(
                [
                    ...
                    "请按以下顺序自然作答：",
                    "1. 先说：这是书中的一个方名或方剂条目。",
                    "2. 再说：当前直接命中的方文里写到了哪些药味、剂量或煎服信息（把引用放在这句末尾）。如果片段不完整，只说“目前检索到这些”。",
                    "3. 如果用户只问“是什么”，不要主动发挥配伍思路、功效概括、证候总结或现代用途；除非证据包已明写且与问句直接相关。",
                ]
            )
            ...
            lines.extend(
                [
                    ...
                    "【高频问法特化：问古文或术语“是什么意思”】",
                    "请按以下顺序自然作答：",
                    "1. 第 1 段（一句话白话解释）：用自然中文，把“它在说什么/提醒什么”先讲明白。禁止先讲系统规则。必须直接回答，用肯定语气。",
                    "2. 第 2 段（解释关键词或关系）：优先解释条文里已经出现的 1~2 个关键词、对举或因果（如“益阳/损阴”“卫阳/荣阴”），尽量沿用书内词语做近距离改写。",
                    "3. 第 3 段（按需）：只有在证据缺口明显时，才用一句话提示回看同段条文、上一句或下一句，并在句末附引用。",
                    "4. 不要引入教材式扩展，例如额外讲脏腑功能、病位、配伍法则、现代术语，除非 Evidence Pack 明写。",
                    "5. 对“太阳病/少阴病是什么意思”这类问法，不要直接下教材式定义；应先说“书里把它写成哪一类脉证或情形”，再解释判断重点。",
                ]
            )
        return "\n".join(lines)
    ...
    if answer_mode == "weak_with_review_notice":
        lines = [
            ...
            "1. 保守解释：先给一个可用的保守理解，必须使用“可能 / 倾向 / 可以先理解为”这类弱化表达，但仍要先回答问题。",
            "2. 不确定原因：接着用一句话说明为什么现在只能弱答（例如：只命中辅助材料、缺原文上下文、片段不完整等）。",
        ...
        if query_style == "meaning_explanation":
            lines.extend(
                [
                    ...
                    "【高频问法特化：问古文或术语“是什么意思”】",
                    "请按以下顺序自然作答：",
                    "1. 第 1 段（一句话白话解释）：用自然中文，把“它在说什么/提醒什么”先讲明白。禁止先讲系统规则。用“可能/倾向/可先理解为”弱化语气直接回答。",
                    "2. 第 2 段（解释关键词或关系）：只解释证据里已经出现的 1~2 个关键词、对举或因果，不要自己补教材式功能术语。",
                    "3. 第 3 段（说明不确定点 + 给核对路径）：用一句话说明不确定的原因，并建议用户回看上一句、下一句或同段条文，并在句末附上引用。",
                    "4. 对“X病是什么意思”这类问法，只能先概括书里写到的脉证或情形，不要直接替它补一条教材式定义。",
                ]
            )
        return "\n".join(lines)
...
def _system_instruction(*, strict_retry: bool) -> str:
    instruction = (
        ...
        "你只能做贴近证据的低阶改写；如果某个概括在证据里没有明说，就不要写。"
        "不要引入教材式扩展，例如额外的病位判断、脏腑功能说明、配伍法则、现代术语，除非 Evidence Pack 明写。"
    ...
    *,
    ...
    blocks.extend(
        [
            (
                "[task]\n"
                "先阅读 Evidence Pack，在心里提炼最重要的 2-4 个证据要点，再写 answer_text。"
            ...
            (
                ...
                "4. strong 模式只能把 primary_evidence 用作正文引用依据；secondary/review 只可帮助你理解上下文，不得在 strong 正文里引用其 [E#]。\n"
                ...
                "16. 绝对不要把段落骨架写成“主依据写到……”“条文说……”“根据 E1……”。依据只能揉进自然解释句里。\n"
                "17. 对解释类问题，优先解释书内已经出现的关键词、对举、因果和提醒，不要自行添加教材式病机概括或术语定义。\n"
                "18. 只输出 JSON，不要输出 markdown 代码块。JSON 至少包含："
