...
ENV_LIMITED_GENERAL_CANARY_LOG_PATH = "RAG_V2_GENERAL_CANARY_LOG_PATH"
DEFAULT_LIMITED_GENERAL_CANARY_LOG_PATH = (
    PROJECT_ROOT
    / "artifacts/data_reconstruction_v2/phase4_5_limited_general_served_canary/runtime_logs_sanitized.jsonl"
)
...
def limited_general_canary_log_path(path_value: str | Path | None = None) -> Path:
    raw = path_value or os.environ.get(ENV_LIMITED_GENERAL_CANARY_LOG_PATH) or DEFAULT_LIMITED_GENERAL_CANARY_LOG_PATH
...
def build_limited_general_canary_log_record(
    *,
    query: str,
    query_type: str,
    ...
    record = {
        ...
        "served_to_general_canary": bool(route_metadata.get("served_to_general_canary")),
        "canary_subject_hash": route_metadata.get("canary_subject_hash") or "",
        "canary_decision_hash": route_metadata.get("canary_decision_hash") or "",
        ...
        "v2_source_citation_fields_present": bool(route_metadata.get("source_citation_fields_present", contract["source_citation_fields_present"])),
...
def append_limited_general_canary_log(record: Mapping[str, Any], *, path: str | Path | None = None) -> Path:
    target = limited_general_canary_log_path(path)
