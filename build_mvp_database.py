...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--schema-draft",
        default="database_schema_draft.json",
        help="Path to database schema draft JSON.",
    )
    parser.add_argument(
        "--policy-json",
        default="layered_enablement_policy.json",
        help="Path to layered enablement policy JSON.",
    )
    parser.add_argument(
        "--db-path",
        default="artifacts/zjshl_mvp.db",
...
def main() -> int:
    args = parse_args()
    root = Path.cwd()
    schema_path = (root / args.schema_draft).resolve()
    policy_path = (root / args.policy_json).resolve()
    db_path = (root / args.db_path).resolve()
    report_path = (root / args.report_path).resolve()
    counts_path = (root / args.counts_path).resolve()
    smoke_checks_path = (root / args.smoke_checks_path).resolve()
